...
### Amazon's category *file names* and each product's embedded `main_category` label are two different vocabularies

An admin scoping an Amazon import to `"Electronics"` got zero results even after that category was added to the fetch list — not a bug, a real quirk of the dataset: the file is named `meta_Electronics.jsonl`, but every record inside it carries `main_category: "All Electronics"`, and `categoryScope` matches against the record's own label, not the file name that happened to produce it (this is deliberate — categoryScope has to mean the same thing across every source, including ones like Open Food Facts with no fixed file-name vocabulary at all). Confirmed live rather than assumed: a ranged fetch of the real file's first record showed the mismatch directly. Not "fixed" by trying to reconcile the two vocabularies — that would just be guessing at one dataset's internal naming — the existing "preview shows the exact name to scope by" UI guidance was already the correct answer; `AMAZON_CATEGORIES` was extended with more real, broadly useful categories (`Electronics`, `Home_and_Kitchen`, `Cell_Phones_and_Accessories`, and others — each checked live before being added) so more of what an admin would plausibly search for is at least discoverable via preview first.
